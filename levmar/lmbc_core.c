		      ...
		      if (firstback) { /*	first backtrack: quadratic fit */
		        ...
		        if (disc > b * b)
			      /* only one positive critical point, must be minimum */
			        tlmbda = (-b + ((a3 < 0)? -(LM_REAL)sqrtf(disc): (LM_REAL)sqrtf(disc))) /a3;
		        else
   ...
   /* compute initial damping factor */
    ...
    while(1){
      ...
      if(jacTeDp<=-rho*pow(Dp_L2, LM_CNST(_POW_)/LM_CNST(2.0))){
        ...
        tmp=(LM_REAL)sqrtf(p_L2); stepmx=LM_CNST(1e3)*( (tmp>=LM_CNST(1.0))? tmp : LM_CNST(1.0) );
